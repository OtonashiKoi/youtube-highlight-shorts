...
- Face-centred symmetry is a release requirement, not a framing preference. On a 1080×1920 canvas, the facial centre must target `x=540`. Measure it from the two-eye midpoint when visible. If eyes are covered, use independently validated facial/head landmarks or manual keyframes; never substitute the centre of the Live2D sprite, shoulder box, crop window, or full source layout.
- Sample the first stable frame, midpoint, final stable frame, both horizontal motion extremes, and frames immediately after every editorial cut. The audit must have at least `start`, `middle`, and `end` samples. Require median absolute facial-centre error `<=24 px`, 95th-percentile error `<=48 px`, and per-sample left/right visible-head margin asymmetry `<=10%` of visible head width.
- Write the measurements to `臉部置中稽核.json` with `time`, `role`, `face_center_x`, `head_left_x`, `head_right_x`, and `method` for every sample, then run `python scripts/validate-face-centering.py --audit 臉部置中稽核.json`. A failure requires retracking, new manual crop keyframes, and a rerender. Do not release a fixed-crop fallback merely because the face remains somewhere inside frame.
